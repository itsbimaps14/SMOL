...
class analisa_qc extends CI_Controller {

	var $folder = 'analisa_qc';
	...
	public function index(){		
		...
		switch ($plant) {
			case 'All':
				$data['read'] = $this->model_qc->view_qc('status_all = "Analisa QC"');
			break;
			default:
				$data['read'] = $this->model_qc->view_qc('plant = "'.$plant.'" AND status_all = "Analisa QC"');
			break;
		}
		$this->template->load('nav_head','qc/'.$this->folder.'/qc_analisa_qc_view',$data);
	}
	...
	public function proses(){
		...
		elseif (isset($_POST['submit'])) {
			...
			$this->template->load('nav_head','qc/'.$this->folder.'/qc_analisa_qc_form',$data);
		}
	}
	...
	public function delete(){
		...
		$this->session->set_flashdata('msg_success','Data kedatangan dengan no running '.$data['record']['no_running_kedatangan'].' berhasil dihapus.');
		redirect('analisa_qc');
	}
}

/* End of file analisa_qc.php */
/* Location: ./application/controllers/qc/analisa_qc.php */