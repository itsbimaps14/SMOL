...
<div class="row">
	<div class="col-lg-10 col-lg-offset-1">
		...
		echo form_open('spek/edit_monitoring','id=edit_release',$hidden);
		?>
...
foreach($release->result() as $r) {?>
	<tr>
		<td class="text-center">
			<input type="hidden" name="id_t_parameter_monitoring[]" value="<?php echo $r->id_t_parameter_monitoring ?>">
			<input type="text" name="kat_spek_monitoring[]" class="form-control" value="<?php echo $r->kat_spek;?>" readonly>
		</td>
		<td class="text-center">
			<input type="text" name="nama_parameter_monitoring[]" class="form-control" value="<?php echo $r->nama_parameter;?>" readonly>
		</td>
		<td class="text-center">
			<input type="text" name="nilai_spek_monitoring[]" class="form-control" value="<?php echo $r->nilai_spek_monitoring;?>">
		</td>
		<td class="text-center">
			<input type="text" name="periode_analisa_monitoring[]" class="form-control" value="<?php echo $r->periode_analisa_monitoring;?>">
		</td>
		<td class="text-center">
			<input type="text" name="titik_sampling_monitoring[]" class="form-control" value="<?php echo $r->titik_sampling_monitoring;?>">
		</td>
		<td class="text-center" colspan="2">
			<?php
			if($i > 0) {
				echo anchor('spek/delete_monitoring/'.$datatab.'/'.$r->id_t_parameter_monitoring,'<i class="fa fa-trash-o"></i>',array('class'=>'btn btn-danger btn-sm',"onclick"=>"return confirm('Anda yakin ingin menghapus?')"));
			}?>
		</td>
	</tr>
	<input type="hidden" name="no_db_spek" value="<?php echo $r->no_db_spek_monitoring?>">
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#r_addRow').click(function() {
			...
			var hidden1i = "<input type='hidden' name='kat_spek_monitoring[]' value='"+hidden1d+"'>";
			...
			row.append($("<td><input type='text' name='opt1[]' class='form-control' value='"+opt1+"' readonly></td>"))
				...
				.append($("<td><input type='text' name='periode_analisa_monitoring[]' class='form-control'></td>"))
				.append($("<td><input type='text' name='titik_sampling_monitoring[]' class='form-control'></td>"))
